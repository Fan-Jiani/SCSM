...
class Prompt(db.Model):
    ...
    chat_id = db.Column(db.String, db.ForeignKey('user.chat_id'), primary_key = True)
    usage_id = db.Column(db.Integer, db.ForeignKey('usage.id'), unique = True, primary_key = True)
    ...
    # build-in functions
    def __init__(self, chat_id, machine_id, start_timestamp, time_to_prompt, ending_time): 
...
class Sensor(db.Model):
    ...
    # build-in functions
    def __init__(self, sensor_id, sensor_type, deployment_date, v_sensor_readings = None, m_sensor_readings = None):
    ...
    def serialize(self):
        if self.sensor_type == 'vibrate_sensor':
